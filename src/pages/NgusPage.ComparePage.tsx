...
export default function NGUsComparePage() {
    ...
    const extraRequired: requiredDataType = [["nguPercentageIncrease"], []];
    ...
    const seconds = NGUs.map((tyNGUs, index) => {
        ...
        return tyNGUs.map((ngu, innerIndex) => {
            return {
                normal: ngu["normal"].calcSecondsToTarget(
                    player.get("total" + ty + "Cap"),
                    player.get("total" + ty + "NGUSpeedFactor"),
                    targets[index][innerIndex]["normal"]
                ),
                evil: ngu["evil"].calcSecondsToTarget(
                    player.get("total" + ty + "Cap"),
                    player.get("total" + ty + "NGUSpeedFactor"),
                    targets[index][innerIndex]["evil"]
                ),
                sadistic: ngu["sadistic"].calcSecondsToTarget(
                    player.get("total" + ty + "Cap"),
                    player.get("total" + ty + "NGUSpeedFactor"),
                    targets[index][innerIndex]["sadistic"]
                ),
            };
        });
    });
        ...
        return tyNGUs.map(function (ngu, innerIndex) {
            ...
            return (
                <tr key={ngu["normal"].key} className={innerIndex % 2 == 0 ? "bg-slate-200 dark:bg-slate-900" : ""}>
                    ...
                    <td
                        className={
                            bigdec_equals(secs["normal"], minTime) ? minbgClass + "px-2 text-right" : "px-2 text-right"
                        }
                    >
                        <span className={bigdec_equals(secs["normal"], minTime) ? "" : "text-red-500"}>
                            {dn(secs["normal"])}
                        </span>
                    </td>
                    <td className={bigdec_equals(secs["normal"], minTime) ? minbgClass + "px" : "px"}>
                        <span className={bigdec_equals(secs["normal"], minTime) ? "" : "text-blue-500"}>
                            {pn(targetLvls["normal"], fmt)}
                        </span>
                    </td>
                    <td
                        className={
                            bigdec_equals(secs["evil"], minTime) ? minbgClass + "px-2 text-right" : "px-2 text-right"
                        }
                    >
                        <span className={bigdec_equals(secs["evil"], minTime) ? "" : "text-red-500"}>
                            {dn(secs["evil"])}
                        </span>
                    </td>
                    <td className={bigdec_equals(secs["evil"], minTime) ? minbgClass + "px" : "px"}>
                        <span className={bigdec_equals(secs["evil"], minTime) ? "" : "text-blue-500"}>
                            {pn(targetLvls["evil"], fmt)}
                        </span>
                    </td>
                    {isSadMode(player.get("gameMode")) ? (
                        <>
                            <td
                                className={
                                    bigdec_equals(secs["sadistic"], minTime)
                                        ? minbgClass + "px-2 text-right"
                                        : "px-2 text-right"
                                }
                            >
                                <span className={bigdec_equals(secs["sadistic"], minTime) ? "" : "text-red-500"}>
                                    {dn(secs["sadistic"])}
                                </span>
                            </td>
                            <td className={bigdec_equals(secs["sadistic"], minTime) ? minbgClass + "px" : "px"}>
                                <span className={bigdec_equals(secs["sadistic"], minTime) ? "" : "text-blue-500"}>
                                    {pn(targetLvls["sadistic"], fmt)}
                                </span>
                            </td>
                        </>
                    ) : null}
    ...
    return (
        <Content
            title="NGUs - Compare which mode is faster"
            infoRequired={infoReq}
            extraRequired={extraReq}
            goRequired={goReq}
            prechildren={<p>This page only makes sense if you are in Evil mode or higher.</p>}
        >
            <p>
                This will tell you what mode is faster for increasing your NGUs. If you state the percentage you want
                above (in Extra Info) it will compare how fast the differents take to achieve that percentage increase.
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            {isSadMode(player.get("gameMode")) ? <th colSpan={2}>Sadistic</th> : null}
                        </tr>
                        <tr>
                            <th>NGU</th>
                            <th>Time</th>
