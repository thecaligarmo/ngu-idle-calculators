...
export default function CookingPage() {
    ...
    if (!_.isEmpty(dish)) {
        ...
        ings.forEach((p: Ingredient) => {
            if (p.isUnlocked) {
                ingList.push(
                    <li key={p.key} className="inline-block w-1/2">
                        <strong>{p.name}</strong>: <span className="text-green-500">{p.amount()}</span>{" "}
                        {p.altAmount() ? <span className="text-violet-500">(alt: {p.altAmount()})</span> : null}
                    </li>
                );
            }
        });
    ...
    return (
        <Content title="Cooking" infoRequired={infoReq} extraRequired={extraReq} goRequired={goReq}>
            <ContentSubsection title="What do I need for 100% meal efficiency?">
                {_.isEmpty(ings) ? (
                    <p>Upload your save file for things to show up</p>
                ) : (
                    <>
                        <p>
                ...
                The amounts in <span className="text-violet-500">purple</span> are for swapping for pairing (see below).
                If you use the alt number for one item, you *must* also use the alt number for its pair.
            </ContentSubsection>
            <ContentSubsection title="Which ingredients are paired?">
                {_.isEmpty(pairs) ? (
                    ...
                    <>
                        <p>
                            Each ingredient comes in a pair which means you can swap the levels and still get max
                            efficiency.
                        </p>
                        <br />
                        <ul>{pairList}</ul>
