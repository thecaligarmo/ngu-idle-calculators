...
export default function Page() {
    ...
    const extraRequired: requiredDataType = [
        ...
        [
            "dailySpinTier",
            "includeDailySpinJackpots",
            "includeValueOfConsumables",
            "moneyPitGoldToss",
            "moneyPitTossesPerDay",
        ],
    ];

    const goRequired: requiredDataType = [
        ["goAP", "goExperience", "goPower", "goQuestDrop", "goRespawn", "goYggdrasilYield"],
    ];
    ...
    const pppPerKill = itopodZone.getPPPPerKill(
        player.get("gameMode"),
        totalPPBonus,
        player.get("bluePill"),
        player.get("blueHeart"),
        player.get("bonusPP")
    );
    const killsPerDay = itopodZone
        .getKillsPerHour(
            player.get("totalPower"),
            getIdleAttackModifier(player.get("spoopySetBonus"), player.get("sadisticNoEquipmentChallenges")),
    ...
    const APFromTower = killsPerDay.divide(itopodZone.cycleLength());
    const EXPFromTower = killsPerDay
        .divide(itopodZone.cycleLength())
        .multiply(bd(itopodZone.exp[0]))
        .multiply(totalExpBonus)
        .divide(bd(100));
    ...
    const apDailySpin = getDailySpinAP(
        player.get("dailySpinTier"),
        totalAPBonus,
        hoursPerDay,
        player.get("includeValueOfConsumables"),
        player.get("includeDailySpinJackpots")
    );

    const totalAPPerDay = apDailySave
        .add(apDailySpin)
        .add(APFromTower)
        .add(apMoneyPit)
        .add(apRebirth)
        ...
        itopod: {
            ...
            extra: (
                <ul>
                    ...
                    <li key="killsPerDay">
                        {pn(killsPerDay, fmt)} kills per {pn(hoursPerDay, fmt)} hours
                    </li>
    ...
    return (
        ...
        >
            <div>
                ...
                <ul>
                    <li key="fruit">
                        We assume that fruit are always harvested at max tier with poop/harvest as in your file.
                    </li>
                    <li key="titanPerk">
                        We assume if you have the Titan EXP Perk, then the bug always applies (as in, every boss gets
                        the 50% boosted exp)
                    </li>
                </ul>
            </div>
