...
import { cookingImportType, ingredientsType } from "@/helpers/types";
...
export class Dish extends Resource {
    ...
    importStats(data: cookingImportType) {
        ...
        this.pairTargets = [data.pair1Target, data.pair2Target, data.pair3Target, data.pair4Target];
    ...
    getScore(ing1: Ingredient, ing2: Ingredient, pairTarget: number): bigDecimal {
        let sum = bd(0);
        if (ing1.isUnlocked) {
            sum = sum.add(this.singleScore(ing1, ing1)).add(this.singleScore(ing2, ing1));
        }
        if (ing2.isUnlocked) {
            sum = sum.add(this.singleScore(ing1, ing2)).add(this.singleScore(ing2, ing2));
            if (ing1.isUnlocked) {
    ...
    singleScore(ing1: Ingredient, ing2: Ingredient): bigDecimal {
        return bd((1 - 0.03 * Math.abs(ing1.targetLvl - ing2.level)) ** 30).multiply(bd(ing1.weight));
    }
    pairScore(ing1: Ingredient, ing2: Ingredient, pairTarget: number): bigDecimal {
        return bd((1 - 0.02 * Math.abs(pairTarget - (ing1.level + ing2.level))) ** 40).multiply(bd(ing1.pairedWeight));
    }
}
