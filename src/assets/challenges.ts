...
import { challengeImportType, propType } from "@/helpers/types";
...
export class Challenge extends Resource {
    ...
    constructor(id: number, key: string, name: string, mode: number, props: propType, extraProps: propType = []) {
    ...
    updateStats() {
        ...
        if (this.extraProps) {
            for (const eprop of this.extraProps) {
                if (!_.isUndefined(eprop[2]) && this.level >= eprop[2]) {
    ...
    importStats(data: challengeImportType) {
...
export const CHALLENGELIST = [
    ...
    new Challenge(
        3,
        ChallengeKeys.ONE_HUNDRED,
        "Normal 100 Levels Challenge",
        GameMode.NORMAL,
        [
            [Stat.ENERGY_WANDOOS_SPEED, 20],
            [Stat.MAGIC_WANDOOS_SPEED, 20],
        ],
        []
    ),
    ...
    new Challenge(
        9,
        ChallengeKeys.NO_NGU,
        "Normal No NGU Challenge",
        GameMode.NORMAL,
        [
            [Stat.ENERGY_NGU_SPEED, 5],
            [Stat.MAGIC_NGU_SPEED, 5],
        ],
        []
    ),
