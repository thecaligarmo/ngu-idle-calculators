...
import { nguImportType, propType } from "@/helpers/types";
...
export class NGU extends Resource {
    ...
    baseCost: bigDecimal;
    constructor(id: number, key: string, name: string, mode: number, props: propType, resource: string, diminishingReturnLevel: number, baseCost: bigDecimal) {
        // level always 0
    ...
    importStats(data: nguImportType): void {
        if (this.id >= 20) {
    ...
    getTargetUsingPercent(percent: bigDecimal | number): bigDecimal {
        ...
        if (this.isRespawn()) {
            // Respawn has weird scaling so we need to fix it.
    ...
    getTargetUsingTime(numMinutes: bigDecimal | number, cap: bigDecimal, speedFactor: bigDecimal): bigDecimal {
        ...
        */
        ...
        const c = numSeconds
            .multiply(bd(2))
            .divide(baseTimePerLevel, roundingDigs)
            .add(curLevel.multiply(curLevel.add(bd(1))));
        const target = bd(Math.sqrt(toNum(bd(1).add(bd(4).multiply(c)))))
            .subtract(bd(1))
            .divide(bd(2), roundingDigs);
    ...
    calcSecondsToTarget(cap: bigDecimal, speedFactor: bigDecimal, target: bigDecimal = bd(-1)): bigDecimal {
        ...
        // n(n+1)/2 - (level)(level+1)/2 = numSeconds / baseTimePerLevel
        const nonBBTime = target
            .multiply(target.add(bd(1)))
            .divide(bd(2))
            .subtract(level.multiply(level.add(bd(1))).divide(bd(2)))
...
export const ENGULIST = [
    ...
    new NGU(
        ...
        GameMode.NORMAL,
        [
            [Stat.ENERGY_WANDOOS_SPEED, 0.1],
            [Stat.MAGIC_WANDOOS_SPEED, 0.1],
        ],
        "energy",
        0,
        bd("2e11")
    ...
    new NGU(
        ...
        GameMode.SADISTIC,
        [
            [Stat.ENERGY_WANDOOS_SPEED, 0.06],
            [Stat.MAGIC_WANDOOS_SPEED, 0.1],
        ],
        "energy",
        1000,
