...
export class Hack extends Resource {
    ...
    importStats(data: hackImportType, playerData: playerImportType): void {
    ...
    getEstimateTimeBetweenLevels(
        ...
        try {
            // If we're doing with a speed change, we go one at a time
            if (this.key == HackKeys.HACK && withSpeedChange) {
                const X = bd(this.baseSpeedDivider).multiply(bd(100)).divide(res3cap.multiply(res3pow));
                let time = bd(0);
                const a = 1.0078;
            ...
            return this.getFullSum(targetLevel)
                .subtract(this.getFullSum(level))
...
export const HACKLIST = [
    ...
    new Hack(13, HackKeys.HACK, "Hack Hack", GameMode.EVIL, [[Stat.HACK_SPEED, 0.05]], 100, 1.1, bd("2e11"), 6757),
