...
import { yggdrasilFruitPermaImportType } from "@/helpers/types";
...
export class Yggdrasil {
    ...
    importStats(data: yggdrasilFruitPermaImportType) {
...
export class FruitOfGold extends Yggdrasil {
    ...
    fruitYield({ firstHarvest = 0, blueHeart = false }: { firstHarvest?: number; blueHeart?: boolean }): bigDecimal {
        ...
        return bd(Math.ceil(Math.ceil(this.tier ** 1.5) * 30 * (1 + firstHarvest / 10) * (this.usePoop ? (blueHeart ? 1.65 : 1.5) : 1)));
    }
    upgradeToTierCost(tier: number): number {
        return tier ** 2;
    }
...
export class FruitOfPowerA extends Yggdrasil {
    ...
    fruitYield({ yieldModifier, firstHarvest = 0, blueHeart = false }: { yieldModifier: bigDecimal; firstHarvest?: number; blueHeart?: boolean }): bigDecimal {
        ...
        return bd(Math.ceil(Math.ceil(this.tier ** 1.5) * (1 + firstHarvest / 10) * (this.usePoop ? (blueHeart ? 1.65 : 1.5) : 1) * toNum(yieldModifier.divide(bd(100)))) ** 1.5);
    }
    upgradeToTierCost(tier: number): number {
        return tier ** 2 * 10;
    }
...
export class FruitOfKnowledge extends Yggdrasil {
    ...
    fruitYield({
        ...
        return bd(
            Math.floor(
                Math.ceil(Math.ceil(this.tier ** 1.5) * (1 + firstHarvest / 10) * (this.usePoop ? (blueHeart ? 1.65 : 1.5) : 1) * toNum(yieldModifier.divide(bd(100))) * 5) *
                    (fokSucksPerk ? 3 : 1) *
...
export class FruitOfLuck extends Yggdrasil {
    ...
    fruitYield({ yieldModifier, firstHarvest = 0, blueHeart = false }: { yieldModifier: bigDecimal; firstHarvest?: number; blueHeart?: boolean }): bigDecimal {
        ...
        return bd(
            Math.ceil(Math.ceil(this.tier ** 1.5) * (1 + firstHarvest / 10) * (this.usePoop ? (blueHeart ? 1.65 : 1.5) : 1) * toNum(yieldModifier.divide(bd(100))) * 0.7) * 0.05
        );
    }
    upgradeToTierCost(tier: number): number {
        return tier ** 2 * 100;
...
export class FruitOfPowerB extends Yggdrasil {
    ...
    fruitYield({ yieldModifier, firstHarvest = 0, blueHeart = false }: { yieldModifier: bigDecimal; firstHarvest?: number; blueHeart?: boolean }): bigDecimal {
        ...
        const levelInc = Math.ceil(Math.ceil(this.tier ** 1.5) * (1 + firstHarvest / 10) * (this.usePoop ? (blueHeart ? 1.65 : 1.5) : 1) * toNum(yieldModifier.divide(bd(100))));
...
export class FruitOfArbitrariness extends Yggdrasil {
    ...
    fruitYield({ apBonus = bd(1), firstHarvest = 0, blueHeart = false }: { apBonus?: bigDecimal; firstHarvest?: number; blueHeart?: boolean }): bigDecimal {
        ...
        return bd(Math.ceil(Math.ceil(this.tier ** 1.5) * (1 + firstHarvest / 10) * (this.usePoop ? (blueHeart ? 1.65 : 1.5) : 1) * 15))
            .multiply(apBonus)
            .divide(bd(100))
            .floor();
    }
...
export class FruitOfNumbers extends Yggdrasil {
    ...
    fruitYield({ yieldModifier, firstHarvest = 0, blueHeart = false }: { yieldModifier: bigDecimal; firstHarvest?: number; blueHeart?: boolean }): bigDecimal {
        ...
        const levelInc = Math.ceil(
            Math.ceil(this.tier ** 1.5) * (1 + firstHarvest / 10) * (this.usePoop ? (blueHeart ? 1.65 : 1.5) : 1) * toNum(yieldModifier.divide(bd(100))) * this.baseSeedFactor
        );
        return this.levelToNum(this.level + levelInc).subtract(this.levelToNum(this.level));
    }
    upgradeToTierCost(tier: number): number {
...
export class FruitOfMacguffinA extends Yggdrasil {
    ...
    fruitYield({
        ...
        const noNGUYieldModifier = yieldModifier.divide(nguYgg);
        return bd(Math.ceil(Math.ceil(this.tier ** 1.5) * (1 + firstHarvest / 10) * (this.usePoop ? (blueHeart ? 1.65 : 1.5) : 1) * toNum(noNGUYieldModifier) * 0.5));
    }
    upgradeToTierCost(tier: number): number {
        return tier ** 2 * 15000;
    }
...
export class FruitOfMacguffinB extends Yggdrasil {
    ...
    fruitYield({
        ...
        const noNGUYieldModifier = yieldModifier.divide(nguYgg);
        return bd(Math.ceil(Math.ceil(this.tier ** 1.5) * (1 + firstHarvest / 10) * (this.usePoop ? (blueHeart ? 1.65 : 1.5) : 1) * toNum(noNGUYieldModifier) * 0.1));
    }
    upgradeToTierCost(tier: number): number {
...
export class FruitOfQuirks extends Yggdrasil {
    ...
    fruitYield({
        ...
        const noNGUYieldModifier = yieldModifier.divide(nguYgg);
        return bd(
            Math.ceil(this.tier * (1 + firstHarvest / 10) * (this.usePoop ? (blueHeart ? 1.65 : 1.5) : 1) * toNum(noNGUYieldModifier) * toNum(qpRewardBonus.divide(bd(100))) * 3)
        );
    }
...
export class FruitOfMayo extends Yggdrasil {
    ...
    fruitYield({ mayoSpeed = bd(1), blueHeart = false }: { mayoSpeed?: bigDecimal; blueHeart?: boolean }): bigDecimal {
        ...
        return bd(this.tier ** 1.1 * (this.usePoop ? (blueHeart ? 1.65 : 1.5) : 1) * toNum(mayoSpeed.divide(bd(100))) * 0.025);
    }
