...
import {
    boostRecyclying,
    getHighestKilledTitanId,
    getIdleAttackModifier,
    totalAPBonus,
    totalCardSpeed,
    totalDropChance,
    totalEnergyCap,
...
export default class Player {
    ...
    importPlayerData(playerData: playerImportType): void {
        ...
        this.set(
            "cardTierAdventure",
            1 +
                (playerData.inventory.itemList.rockLobsterComplete ? 1 : 0) +
        ...
        this.set(
            ...
            playerData.challenges.noRebirthChallenge.curCompletions +
        ...
        this.set(
            ...
            1 +
                ...
                (playerData.inventory.itemList.pinkHeartComplete ? 1 : 0)
        );
    ...
    updateTotals(): void {
        ...
        this.set("itopodFloor", itopodZone.getOptimalFloor(totalPower(this), idleAttackModifier));
    }

    setAdvTrainings(playerData: playerImportType): void {
    ...
    setApItems(playerData: playerImportType): void {
        ...
        if (playerData.arbitrary.energyPotion2InUse) apToAdd["energyPotionB"] = 1;
        if (playerData.arbitrary.magicPotion2InUse) apToAdd["magicPotionB"] = 1;
        if (playerData.arbitrary.res3Potion2InUse) apToAdd["resource3PotionB"] = 1;
        ...
        if (playerData.arbitrary.boughtAutoNuke) apToAdd["autoNuker"] = 1;
        ...
        if (playerData.arbitrary.hasAcc7) apToAdd["yetAnotherExtraAccessorySlot"] = 1;
        if (playerData.arbitrary.hasAcc8) apToAdd["anEvilAccessorySlot"] = 1;
        if (playerData.arbitrary.hasAcc9) apToAdd["extraAccessorySlot2"] = 1;
        if (playerData.arbitrary.hasYggdrasilReminder) apToAdd["yggdrasilHarvestLight"] = 1;
        if (playerData.arbitrary.hasExtendedSpinBank) apToAdd["7DayTimeBankforDailySpin"] = 1;
        ...
        if (playerData.arbitrary.hasCubeFilter) apToAdd["filterBoostsintoInfinityCube"] = 1;
        if (playerData.arbitrary.hasDaycareSpeed) apToAdd["daycareSpeedBoost"] = 1;
        if (playerData.arbitrary.boughtLazyITOPOD) apToAdd["lazyITOPODFloorShifter"] = 1;
        ...
        if (playerData.arbitrary.hasQuestLight) apToAdd["questReminder"] = 1;
        if (playerData.arbitrary.hasFasterQuests) apToAdd["fasterQuesting"] = 1;
        if (playerData.arbitrary.hasExtendedQuestBank) apToAdd["extendedQuestBank"] = 1;
        if (playerData.arbitrary.wishSpeedBoster) apToAdd["fasterWishes"] = 1;
        if (playerData.arbitrary.gotTagslot1) apToAdd["extraTagSlot"] = 1;
        ...
        if (playerData.arbitrary.advLightBought) apToAdd["adventureLight"] = 1;
        if (playerData.arbitrary.advAdvancerBought) apToAdd["adventureAdvancer"] = 1;
        if (playerData.arbitrary.goToQuestZoneBought) apToAdd["goToQuestZoneButton"] = 1;
        if (playerData.arbitrary.hasCubeFilter) apToAdd["filterBoostsintoInfinityCube"] = 1;
    ...
    setBeards(playerData: playerImportType): void {
        const beards: Beard[] = [];
    ...
    setCards(playerData: playerImportType): void {
        const cards: Card[] = [];
    ...
    setChallenges(playerData: playerImportType): void {
    ...
    setDish(playerData: playerImportType): void {
        const dish: Dish = DISHES[playerData.cooking.curDishIndex];
        dish.importStats(playerData.cooking);
        this.set("dish", dish);
    }

    setEquipmentWeapon(playerData: playerImportType): void {
        const weapon = playerData.inventory.weapon;
        const weaponItem: Item = _.cloneDeep(ITEMS[weapon.id]);
        weaponItem.importStats(weapon);
        this.set("equipmentWeapon", weaponItem);
    }
    setEquipmentWeaponTwo(playerData: playerImportType): void {
        const weaponTwo = playerData.inventory.weapon2;
        if (weaponTwo.id != 0) {
            const weaponTwoItem: Item = _.cloneDeep(ITEMS[weaponTwo.id]);
            // weapon 2 doesn't necessarily have a full ratio
            weaponTwoItem.ratio = playerData.wishes.wishes[28].level * 0.05 + playerData.wishes.wishes[45].level * 0.05;
    ...
    setEquipmentHead(playerData: playerImportType): void {
        const head = playerData.inventory.head;
        const headItem: Item = _.cloneDeep(ITEMS[head.id]);
        headItem.importStats(head);
        this.set("equipmentHead", headItem);
    }
    setEquipmentChest(playerData: playerImportType): void {
        const chest = playerData.inventory.chest;
        const chestItem: Item = _.cloneDeep(ITEMS[chest.id]);
        chestItem.importStats(chest);
        this.set("equipmentChest", chestItem);
    }
    setEquipmentLegs(playerData: playerImportType): void {
        const legs = playerData.inventory.legs;
        const legsItem: Item = _.cloneDeep(ITEMS[legs.id]);
        legsItem.importStats(legs);
        this.set("equipmentLegs", legsItem);
    }
    setEquipmentBoots(playerData: playerImportType): void {
        const boots = playerData.inventory.boots;
    ...
    setEnergyNGUs(playerData: playerImportType): void {
    ...
    setMagicNGUs(playerData: playerImportType): void {
        const magicNGUs: NGU[] = [];
    ...
    setHacks(playerData: playerImportType): void {
        const hacks: Hack[] = [];
        playerData.hacks.hacks.forEach((hackData, index) => {
            if (index < 15) {
    ...
    setMacguffins(playerData: playerImportType): void {
    ...
    setPerks(playerData: playerImportType): void {
        const perks: Perk[] = [];
    ...
    setQuirks(playerData: playerImportType): void {
        const quirks: Quirk[] = [];
    ...
    setTitans(playerData: playerImportType): void {
        const titans: Titan[] = [];
        Object.values(Titans).forEach((titan) => {
    ...
    setWandoos(playerData: playerImportType): void {
    ...
    setItemSets(playerData: playerImportType): void {
        const itemSets: { [key: string]: ItemSet } = {};
        for (const set of Object.values(ItemSets)) {
            set.importStats(playerData);
            itemSets[set.key] = _.cloneDeep(set);
        }
        this.set("itemSets", itemSets);
    }
    setMaxxedItems(playerData: playerImportType): void {
        const maxxedItemIds: number[] = [];
        playerData.inventory.itemList.itemMaxxed.forEach((maxxed: boolean, item: number) => {
            if (maxxed) {
