...
export default function Page() {
    ...
    var infoRequired = [['totalRespawnTime', 'totalPPBonus%', 'totalPower']]

    // Set extra required (not from playerData)
    var extraRequired = [['itopodFloor', 'bluePill^']]
    ...
    var pppPerKill = v('totalPPBonus%')
                        .divide(bd(100))
                        .multiply(v('bluePill^').compareTo(bd(1)) == 0 ? bd(2) : bd(1))
                        .multiply(bd(itopodFloor + 200))
                        .floor()

    // PPP Per Day
    // @todo Still need to add evil/sadistic and blue Heart
    // PP Bonus -> stats
    // * 
    ...
    var killsPerDay = bd(24 * 60 * 60).divide( respawnTime.add(idleAttackCooldown))



    return (
        <Content title="Daily Gains" infoRequired={infoReq} extraRequired={extraReq}>
            <ContentSubsection title="How many PP do I get in a day?">
                <ul>
                    <li key="pppPerKill">{pn(pppPerKill, fmt)} ppp / kill on floor {pn(bd(itopodFloor), fmt)}</li>
                    <li key="killsPerDay">{pn(killsPerDay, fmt)} kills / day (assuming 1-hit per kill)</li>
                    <li key="total" className="mt-2 border-white border-t-2 border-solid"><span className="text-red-500">{pn(killsPerDay.multiply(pppPerKill).divide(bd(1000000)), fmt, 2)}</span> <strong>PP per Day</strong> (assuming 1-hit per kill)</li>
                </ul>
