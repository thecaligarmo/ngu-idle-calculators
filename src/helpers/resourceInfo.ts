...
function globalDiggerBonus(player: Player): number {
    ...
    if (diggers.length) {
        totalLevel = Object.values(diggers).reduce((curVal: number, d: Digger) => {
            return d.maxLevel + curVal;
        }, 0);
    }
    const challenges: Challenge[] = player.get("challenges");
    const challengeBonus: number =
        !_.isUndefined(challenges[GameMode.NORMAL]) &&
        !_.isUndefined(challenges[GameMode.NORMAL][10]) &&
        challenges[GameMode.NORMAL][10].level > 0
            ? 5
            : 0;
...
export function isCompletedChallenge(player: Player, key: string, mode: number, level: number): boolean {
    ...
    const chosenChallenge =
        !_.isUndefined(challenges[mode]) && !_.isUndefined(challenge) && challenges[mode][challenge.id % 100];
