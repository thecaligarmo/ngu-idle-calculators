...
export function describeStat(data: any, fmt: string, isRespawn: boolean = false) {
    const rows = Object.keys(data).map((k) => {
        ...
        if (k == "total") {
            ...
            return (
                <li key={k} className="mt-2 border-white border-t-2 border-solid">
                    <strong>Total:</strong>{" "}
                    <span className="text-red-500">
                        {pn(data[k].val, fmt, sigFigs)}
                        {percent}
                    </span>
                    {extra}
                </li>
            ...
            return (
                <li key={k} className="border-white border-t-2 border-solid">
                    Subtotal: {pn(data[k].val, fmt, sigFigs)}
                </li>
            );
        }
        if (isRespawn) {
        ...
        return (
            <li key={k}>
                {data[k].name} ({pn(data[k].val, fmt, sigFigs)}
                {percent})
            </li>
        );
    });

    return <ul className="ml-5">{rows}</ul>;
}
...
export function getStatInfo(player: Player) {
    return {
        ...
        enguSpeed: {
            ...
            beard: {
                name: "x Beard",
                val: beardInfoPerm(player, Stat.ENERGY_NGU_SPEED)
                    .divide(bd(100))
                    .multiply(beardInfoTemp(player, Stat.ENERGY_NGU_SPEED).divide(bd(100)))
                    .multiply(bd(100))
                    .round(),
            },
            digger: {
        ...
        eBeards: {
            ...
            countDiv: {
                name: "/ Count Divider",
                val: bigdec_max(
                    activeBeards(player, "energy").multiply(
                        isMaxxedItemSet(player, ItemSets.BEARDVERSE) ? bd(0.9) : bd(1)
                    ),
                    bd(1)
                ),
                sigFig: 1,
