...
describe("Resource Info Helper", () => {
    describe("Achievement Info", () => {
        ...
        test.each(cases)("%s", (_name, player, expectedValues) => {
            expectClose(achievementAPBonus(player), expectedValues);
        });
    });

    describe("Adv Training Info", () => {
        ...
        test.each(cases)("%s", (_name, player, expectedValues) => {
            expectClose(advTrainingInfo(player, Stat.POWER), expectedValues["power"]);
            expectClose(advTrainingInfo(player, Stat.ENERGY_POWER), expectedValues["epower"]);
        });
    });
    ...
    describe("AP Info", () => {
        ...
        test.each(cases)("%s", (_name, player, expectedValues) => {
            expectClose(apItemInfo(player, Stat.POWER), expectedValues);
        });
    });

    describe("Beard Temp Info", () => {
        ...
        test.each(cases)("%s", (_name, player, expectedValues) => {
            expectClose(beardInfoTemp(player, Stat.POWER, true), expectedValues["power"]);
            expectClose(beardInfoTemp(player, Stat.DROP_CHANCE, true), expectedValues["dc"]);
        });
    });

    describe("Beard Perm Info", () => {
        const cases: [string, Player, Pick<Required<infoObjType>, "power" | "dc">][] = [
            ["Early Normal 1", earlyNormalPlayer, { power: 100, dc: 100 }],
            ["Early Normal 2", earlyNormalTwoPlayer, { power: 100, dc: 100 }],
        ...
        test.each(cases)("%s", (_name, player, expectedValues) => {
            expectClose(beardInfoPerm(player, Stat.POWER), expectedValues["power"]);
            expectClose(beardInfoPerm(player, Stat.DROP_CHANCE), expectedValues["dc"]);
        });
    });

    describe("Challenge Info", () => {
        ...
        test.each(cases)("%s", (_name, player, expectedValues) => {
            expectClose(challengeInfo(player, Stat.POWER), expectedValues["power"]);
            expectClose(challengeInfo(player, Stat.MAGIC_NGU_SPEED), expectedValues["mngu"]);
        });
    });

    describe("Digger Info", () => {
        ...
        test.each(cases)("%s", (_name, player, expectedValues) => {
            expectClose(diggerInfo(player, Stat.POWER), expectedValues["power"]);
        });
    });
    ...
    describe("Equipment Info", () => {
        const cases: [
            ...
            Player,
            Pick<
                Required<infoObjType>,
                "power" | "toughness" | "epower" | "mcap" | "dc" | "respawn" | "engu" | "mwandoos"
            >
        ][] = [
            [
                "Early Normal 1",
        ...
        test.each(cases)("%s", (_name, player, expectedValues) => {
            expectClose(equipmentWithCubeInfo(player, Stat.POWER), expectedValues["power"]);
            expectClose(equipmentWithCubeInfo(player, Stat.TOUGHNESS), expectedValues["toughness"]);
            expectClose(equipmentWithCubeInfo(player, Stat.ENERGY_POWER), expectedValues["epower"]);
            expectClose(equipmentWithCubeInfo(player, Stat.MAGIC_CAP), expectedValues["mcap"]);
            expectClose(equipmentWithCubeInfo(player, Stat.DROP_CHANCE), expectedValues["dc"]);
            expectClose(equipmentWithCubeInfo(player, Stat.RESPAWN), expectedValues["respawn"]);
            expectClose(equipmentWithCubeInfo(player, Stat.ENERGY_NGU_SPEED), expectedValues["engu"]);
            expectClose(equipmentWithCubeInfo(player, Stat.MAGIC_WANDOOS_SPEED), expectedValues["mwandoos"]);
        });
    });
    ...
    describe("Hack Info", () => {
        ...
        test.each(cases)("%s", (_name, player, expectedValues) => {
            expectClose(hackInfo(player, Stat.POWER), expectedValues["power"]);
            expectClose(hackInfo(player, Stat.QUEST_REWARD), expectedValues["qp"]);
            expectClose(hackInfo(player, Stat.WISH_SPEED), expectedValues["wish"]);
        });
    });

    describe("Macguffin Info", () => {
        const cases: [
            string,
            Player,
            Pick<Required<infoObjType>, "power" | "epower" | "dc" | "attack"> & Partial<infoObjType>
        ][] = [
            [
                "Early Normal 1",
                earlyNormalPlayer,
        ...
        test.each(cases)("%s", (_name, player, expectedValues) => {
            expectClose(macguffinInfo(player, Stat.POWER), expectedValues["power"]);
            expectClose(macguffinInfo(player, Stat.ENERGY_POWER), expectedValues["epower"]);
            expectClose(macguffinInfo(player, Stat.DROP_CHANCE), expectedValues["dc"]);
            expectClose(macguffinInfo(player, Stat.ATTACK), expectedValues["attack"]);
            if ("blood" in expectedValues) {
                expectClose(macguffinInfo(player, Stat.BLOOD), expectedValues["blood"]);
            }
        });
    });
    ...
    describe("NGU Info", () => {
        ...
        test.each(cases)("%s", (_name, player, expectedValues) => {
            expectClose(nguInfo(player, Stat.POWER), expectedValues["power"]);
            expectClose(nguInfo(player, Stat.YGGDRASIL_YIELD), expectedValues["ygg"]);
            expectClose(nguInfo(player, Stat.DROP_CHANCE), expectedValues["dc"]);
            expectClose(nguInfo(player, Stat.ATTACK), expectedValues["attack"]);
            expectClose(nguInfo(player, Stat.ENERGY_NGU_SPEED), expectedValues["engu"]);
        });
    });
    ...
    describe("Perk Info", () => {
        ...
        test.each(cases)("%s", (_name, player, expectedValues) => {
            expectClose(perkInfo(player, Stat.POWER), expectedValues["power"]);
            expectClose(perkInfo(player, Stat.BOOSTS_BOOST), expectedValues["boosts"]);
            expectClose(perkInfo(player, Stat.ENERGY_POWER), expectedValues["epower"]);
        });
    });
    ...
    describe("Quirk Info", () => {
        ...
        test.each(cases)("%s", (_name, player, expectedValues) => {
            expectClose(quirkInfo(player, Stat.POWER), expectedValues["power"]);
            expectClose(quirkInfo(player, Stat.ENERGY_CAP), expectedValues["ecap"]);
            expectClose(quirkInfo(player, Stat.MAGIC_NGU_SPEED), expectedValues["mngu"]);
        });
    });
    ...
    describe("Wish Info", () => {
        ...
        test.each(cases)("%s", (_name, player, expectedValues) => {
            expectClose(wishInfo(player, Stat.POWER), expectedValues["power"]);
            expectClose(wishInfo(player, Stat.RES3_POWER), expectedValues["rpower"]);
            expectClose(wishInfo(player, Stat.HACK_SPEED), expectedValues["hack"]);
        });
    });

    describe("Item Set Info", () => {
        const cases: [string, Player, ItemSet[], ItemSet[], number][] = [
            ...
            [
                "Mid Evil 1",
                midEvilPlayer,
                [ItemSets.PRETTY, ItemSets.NERD],
                [ItemSets.CONSTRUCTION, ItemSets.NETHER],
                10,
            ],
            ["Mid Evil 2", midEvilTwoPlayer, [ItemSets.PRETTY, ItemSets.NERD], [ItemSets.PIRATE], 10],
        ];
        test.each(cases)("%s", (_name, player, expectSets, noExpectSets, numQuests) => {
