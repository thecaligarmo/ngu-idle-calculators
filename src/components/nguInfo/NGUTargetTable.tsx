...
export default function NGUTargetTable({ type, NGUs, targets, seconds, totalSeconds, fmt }: NGUProps): ReactNode {
    ...
    for (const index in NGUs) {
        ...
        if (
            ngu.key == "NGUWandoos" ||
            ngu.key == "NGUAdventureA" ||
            ngu.key == "NGUDropChance" ||
            ngu.key == "NGUMagicNGU" ||
            ngu.key == "NGUYggdrasil" ||
            ngu.key == "NGUTimeMachine" ||
            ngu.key == "NGUEnergyNGU"
        ) {
            precision = 1;
        }
        if (ngu.key == "NGURespawn" || ngu.key == "NGUPP" || ngu.key == "NGUExp" || ngu.key == "NGUAdventureB") {
            precision = 2;
        }
