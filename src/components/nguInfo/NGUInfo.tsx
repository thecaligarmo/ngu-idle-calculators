...
export default function NGUInfo({ gameMode }: NGUProps) {
        ...
        return tyNGUs.map((ngu) => {
            switch (calcType) {
                ...
                case NGU_TIME:
                    ...
                    return ngu.getTargetUsingTime(
                        player.get("nguTimeInMinutes"),
    ...
    const topButtons = (
        <>
            ...
            <ChoiceButton
                text="Using Targets"
                onClick={() => setCalcType(NGU_TARGET)}
                active={calcType == NGU_TARGET}
            />
            <ChoiceButton
                text="Using Percentage of Current Value"
                onClick={() => setCalcType(NGU_PERCENTAGE)}
                active={calcType == NGU_PERCENTAGE}
            />
            <ChoiceButton
                text="Using Certain Value"
                onClick={() => setCalcType(NGU_VALUE)}
                active={calcType == NGU_VALUE}
            />
    ...
    return (
        <Content
            ...
            title={"NGUs - " + (modeText == "" ? "Normal" : modeText)}
        ...
        >
            <ContentSubsection title="How long until I reach targets?">
                <p>
                    The following tables let you know how much <span className="text-red-500">Time</span> is needed
                    until you get to the <span className="text-blue-500">Target</span>. The Value represents the bonus
                    you will have once you reach the target.
                </p>
            ...
            <ContentSubsection title="How much energy/magic is needed to cap/max the bar at target level?">
                <p>
                    The following tells you how much energy or magic you will need to{" "}
                    <span className="text-red-500">Cap/BB</span> the bar at the{" "}
                ...
                <NGUCapTable
                    ...
                    NGUs={NGUs[0]}
                    targets={targets[0]}
                    speedFactor={player.get("totalEnergyNGUSpeedFactor")}
                    fmt={fmt}
                />
                <NGUCapTable
                    type="Magic"
                    NGUs={NGUs[1]}
                    targets={targets[1]}
                    speedFactor={player.get("totalMagicNGUSpeedFactor")}
                    fmt={fmt}
                />
            </ContentSubsection>
            <ContentSubsection title="How much energy is needed to cap/max the bar for 24 hours non-stop?">
                <p>
                    The following tells you how much energy or magic you will need to{" "}
                    <span className="text-red-500">Cap</span> the bar for a continuous 24 hours.
                </p>
